...
/** POST /api/radar/campaigns/[id]/pause */
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        const { data, error } = await supabase
            .from('campaigns')
            .update({ status: 'paused', version: campaign.version + 1 })
            .eq('id', id)
            .select()
            .single();

        if (error) return NextResponse.json({ error: 'Failed to pause campaign' }, { status: 500 });
