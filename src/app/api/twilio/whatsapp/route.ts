/**
 * Twilio WhatsApp webhook. Receives inbound WhatsApp messages and responds with AI.
 * Configure in Twilio: Messaging -> WhatsApp Sandbox/Sender -> Webhook URL -> POST to this route.
 *
 * Flow:
 * 1. User sends "START-<short_code>" to our WhatsApp number
 * 2. We find demo by whatsapp_short_code
 * 3. Find/create lead by WaId (or From)
 * 4. Process message through chat engine with channel 'messenger'
...
export async function POST(request: NextRequest) {
    ...
    const { allowed } = checkRateLimit(`twilio-whatsapp:${ip}`, CHAT_LIMIT);
    if (!allowed) {
    ...
    // WaId is the WhatsApp ID (phone number); From is whatsapp:+1234567890
    const waId = body.WaId || body.From?.replace(/^whatsapp:/i, '') || body.From || body.from;
    ...
    if (shortCode) {
        const { data: demo } = await supabase
            ...
            .eq('whatsapp_short_code', shortCode)
            .single();
    ...
    if (!demoId) {
        return new NextResponse(
            '<?xml version="1.0" encoding="UTF-8"?><Response><Message>To start, send START followed by your demo code (e.g. START-abc12345). Get your code from the demo link.</Message></Response>',
            { headers: { 'Content-Type': 'text/xml' } }
        );
    }
    ...
    try {
        const lead = await findOrCreateLead(supabase, demoId, waId);
