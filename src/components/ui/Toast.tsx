...
function ToastContainer() {
    ...
    const variants = {
        ...
        success: 'bg-surface border-l-4 border-l-success border-border',
        error: 'bg-surface border-l-4 border-l-error border-border',
        warning: 'bg-surface border-l-4 border-l-warning border-border',
    };
...
export function ToastItem({ title, description, variant = 'default', onClose }: ToastItemProps) {
    ...
    return (
        <div className={`flex items-start p-4 rounded-lg shadow-lg border ${variants[variant]}`} role="alert">
            ...
            {onClose && (
                <button onClick={onClose} className="ml-3 text-foreground-muted hover:text-foreground-secondary">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
